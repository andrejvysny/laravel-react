...
function TransactionList({transactions, monthlySummaries}:Props ) {
    ...
    return (
        <div className="flex flex-col gap-0 max-w-xl mx-auto w-full">
            {sortedMonths.map(month => {
                ...
                return (
                    <div key={month} className="flex flex-col gap-2 mb-10 pb-10 border-b-1 border-accent">
                        {/* Summary at the top of the month */}
                        <div className="flex items-center gap-4 bg-gray-900 p-4 rounded-xl mb-4">
                            <div className="flex-1">
                                <div className="font-medium">{month}</div>
                                <div className="flex gap-4 mt-1">
                                    <span className="text-sm text-gray-400">Income: <span
                                        className="text-green-500">+{summary.income.toFixed(2)}€</span></span>
                                    <span className="text-sm text-gray-400">Expense: <span
                                        className="text-red-500">-{summary.expense.toFixed(2)}€</span></span>
