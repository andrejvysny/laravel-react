...
export default function CleanStep({ data, onComplete }: CleanStepProps) {
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <h3 className="text-xl font-semibold mb-4 text-foreground">Clean your data</h3>
            <p className="mb-6 text-foreground">
                Review your data before proceeding. You can remove any rows that you don't want to import.
            </p>

            <div className="rounded-lg border border-foreground overflow-x-auto mb-6">
                <table className="w-full">
                    <thead className="bg-foreground border-b border-foreground">
                        <tr>
                        ...
                        {cleanedData.map((row, index) => (
                            <tr key={index} className="border-b border-muted-foreground">
                                {Object.entries(row).filter(([key]) => !key.startsWith('_')).map(([key, value]) => (
                                    <td key={key} className="px-4 text-foreground py-2 whitespace-nowrap truncate">
